...
export const AppDataSource = new DataSource({
  ...
  // 개발 환경('DEV' 또는 'test')일 때만 스키마 자동 동기화 (프로덕션에서는 매우 위험하므로 false)
  synchronize: envConfig.NODE_ENV === 'DEV' || envConfig.NODE_ENV === 'test',
  // 개발 환경('DEV' 또는 'test')일 때만 쿼리 로그 출력
  logging: envConfig.NODE_ENV === 'DEV' || envConfig.NODE_ENV === 'test',
